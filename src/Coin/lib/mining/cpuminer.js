...
class CPUMiner extends EventEmitter {
  ...
  findNonce(job) {
    ...
    while (max <= 0xffffffff) {
      // console.log(data, target, min, max)
  ...
  async findNonceAsync(job) {
    console.log("find nonce at cpu 323")
    if (!this.workers)
  ...
  async mineAsync(job) {
    let nonce;
    console.log("mine async at cpu 388")
    ...
    console.log("found nonce cpu 406")
  ...
  sendStatus(job, nonce) {
    ...
    console.log("send status cpu 421")
